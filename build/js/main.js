...
var checkList = document.querySelectorAll('.checkbox__style');
var scorePlace = document.getElementById('score'); //градусы на один 'шаг' исходя из 180 на полукруг

var step = 180 / checkList.length; // кол-во отмеченных навыков
...
;
var exTotalChecked = calcChecked(); //функция - анимация счётчика 
...
function scoreGrowing(from, to) {
  ...
  if (from < to) {
    var t = Math.round(time / (to / scoreStep));
    ...
    var _t = Math.round(time / (from / scoreStep));
...
window.onload = function loadWind() {
  ...
  var from = 125 * (totalChecked - 1);
  var to = 125 * totalChecked;
...
checkList.forEach(function (element) {
  element.addEventListener('click', function (event) {
    var totalChecked = calcChecked(); //анимация стрелки по клику
